...
object Graph {
  ...
  /** Factory method for default graph implementation.
    ...
    * */
  def apply(connectionData: ConnectionData): Graph = AdjacencyList(connectionData)
}
...
private[model] object AdjacencyList {
  ...
  /** Factory method to create an [[AdjacencyList]] from [[ConnectionData]].
    *
    * @note This is graph implementation specific, thus needs to be close to the implementation as a factory method.
    * */
  def apply(connectionData: ConnectionData): AdjacencyList = {
    def findRelationships(p: Person): HashSet[Person] = {
      val personConnections: Seq[Person] = connectionData.relationships.flatMap(_.findRelatedTo(p))
      HashSet.from(personConnections)
    }
    val personToConnections: Seq[(Person, HashSet[Person])] = connectionData.people.map(p => p -> findRelationships(p))
